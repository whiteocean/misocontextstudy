...
disp('Select Block 1 Physio Dataset.')
...
BASEef = BASEst + min(min(GSRbaseS(2:end,:)));
CLIPef = CLIPst + min(min(GSRclipS(2:end,:)));
ITIVef = ITIVst + min(min(GSRitivS(2:end,:)));
...
plot(hax5, GSRfbase{b})
plot(hax6, GSRfclip{b})
plot(hax7, GSRfitiv{b})
